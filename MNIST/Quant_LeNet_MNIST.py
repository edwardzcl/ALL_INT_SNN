...
print(args.k, args.use_ternary, args.resume, args.mode)
...
def model(x, use_ternary, is_train=True, reuse=False):
    ...
    with tf.variable_scope("binarynet", reuse=reuse):
        ...
        #net = tl.layers.Quant_Layer(net, k)
        net = tl.layers.Quant_Layer1(net)
        #net = tl.layers.SignLayer(net)
        #net.outputs = (net.outputs+1)/2
        if use_ternary:
           net = tl.layers.Quant_Conv2d(net, n_filter=32, filter_size=(2, 2), strides=(2, 2), padding='SAME', b_init=None, name='bcnn2')  
        else:
        ...
        #net.outputs = (net.outputs+1)/2
        if use_ternary:
           net1 = tl.layers.Quant_Conv2d(net, n_filter=64, filter_size=(5, 5), strides=(1, 1), padding='VALID', b_init=None, name='bcnn3')
        else:
        ...
        #net.outputs = (net.outputs+1)/2
        if use_ternary:
           net = tl.layers.Quant_Conv2d(net, n_filter=64, filter_size=(2, 2), strides=(2, 2), padding='SAME', b_init=None, name='bcnn4')
        else:
        ...
        #net.outputs = (net.outputs+1)/2
        if use_ternary:
           net = tl.layers.Quant_Conv2d(net, n_filter=1000, filter_size=(4, 4), strides=(1, 1), padding='VALID', b_init=None, name='fc1')
        else:
...
# define inferences
net_train, _ = model(x, args.use_ternary, is_train=True, reuse=False)
net_test,net1 = model(x, args.use_ternary, is_train=False, reuse=True)
