...
class MyNet():
    def __init__(self):
        self.conv0 = Conv2d(in_channels=3, n_filter=96, filter_size=(3, 3), padding=1, stride=1, \
                            quant_level = args.k, first_layer=True)   
        self.bn0 = BatchNorm2d(n_channel=96, momentum=0.1, first_layer=True)
        self.relu0 = Relu()

        self.conv1 = Conv2d(in_channels=96, n_filter=256, filter_size=(3, 3), padding=1, stride=1, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)   
        self.bn1 = BatchNorm2d(n_channel=256, momentum=0.1, use_int=args.use_int)
        self.relu1 = Relu()

        self.conv2 = Conv2d(in_channels=256, n_filter=256, filter_size=(2, 2), padding=0, stride=2, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn2 = BatchNorm2d(n_channel=256, momentum=0.1, use_int=args.use_int)
        self.relu2 = Relu()

        self.conv3 = Conv2d(in_channels=256, n_filter=384, filter_size=(3, 3), padding=1, stride=1, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn3 = BatchNorm2d(n_channel=384, momentum=0.1, use_int=args.use_int)
        self.relu3 = Relu()

        self.conv4 = Conv2d(in_channels=384, n_filter=384, filter_size=(2, 2), padding=0, stride=2, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn4 = BatchNorm2d(n_channel=384, momentum=0.1, use_int=args.use_int)
        self.relu4 = Relu()

        self.conv5 = Conv2d(in_channels=384, n_filter=384, filter_size=(3, 3), padding=1, stride=1, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn5 = BatchNorm2d(n_channel=384, momentum=0.1, use_int=args.use_int)
        self.relu5 = Relu()

        self.conv6 = Conv2d(in_channels=384, n_filter=256, filter_size=(3, 3), padding=1, stride=1, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn6 = BatchNorm2d(n_channel=256, momentum=0.1, use_int=args.use_int)
        self.relu6 = Relu()

        self.conv7 = Conv2d(in_channels=256, n_filter=256, filter_size=(2, 2), padding=0, stride=2, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn7 = BatchNorm2d(n_channel=256, momentum=0.1, use_int=args.use_int)
        self.relu7 = Relu()

        self.conv8 = Conv2d(in_channels=256, n_filter=1024, filter_size=(3, 3), padding=0, stride=1, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn8 = BatchNorm2d(n_channel=1024, momentum=0.1, use_int=args.use_int)
        self.relu8 = Relu()

        self.conv9 = Conv2d(in_channels=1024, n_filter=1024, filter_size=(1, 1), padding=0, stride=1, \
                            quant_level = args.k, use_ternary=args.use_ternary, use_int=args.use_int)
        self.bn9 = BatchNorm2d(n_channel=1024, momentum=0.1, use_int=args.use_int)
        self.relu9 = Relu()

        self.flatten = Flatten()
        
        self.fc1 = Linear(dim_in=1024, dim_out=10)
    ...
    def forward(self, X, t, noise_ratio, mode):
        ...
        conv9_sop_num
        abs_spike = np.sum(np.abs(conv1_out)) + np.sum(np.abs(conv2_out)) + np.sum(np.abs(conv3_out)) + np.sum(np.abs(conv4_out)) + \
        np.sum(np.abs(conv5_out)) + np.sum(np.abs(conv6_out)) + np.sum(np.abs(conv7_out)) + np.sum(np.abs(conv8_out)) + np.sum(np.abs(conv9_out))
        sum_spike = np.sum(conv1_out) + np.sum(conv2_out) + np.sum(conv3_out) + np.sum(conv4_out) + \
        np.sum(conv5_out) + np.sum(conv6_out) + np.sum(conv7_out) + np.sum(conv8_out) + np.sum(conv9_out)
...
def test(sess, use_ternary, use_int, test_images, quant_level, test_labels, network, n_data, batch_size, time_step, noise_ratio):
    """
    ...
    for i in range(0, n_data, batch_size):
        ...
        #batch_datas = batch_datas.transpose(0, 3, 1, 2)
...
with tf.device('/cpu:0'):
    ...
    test_acc = test(sess, args.use_ternary, args.use_int, x_test_batch, args.k, y_test_batch, network=mynet, n_data=y_test.shape[0], batch_size=batch_size, time_step=time_step, noise_ratio=args.noise_ratio)
